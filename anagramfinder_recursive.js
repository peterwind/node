...
function findSecretPhrase(wordlist) {
	...
	//optimizedWordArray = ['pastils', 'turnout', 'towy'];
...
// Variables used in the recursive exploration of the secret phrase
var found = false;
var secretPhrase;
 ...
 *
 * Note:
 * The algorithm only searches to a certain depth (MAX_SEARCH_DEPTH)
 * 
...
function extractSecretPhrase(wordArray) {
	...
	// Find the relevant combinations recursively
	recursivelyFindSecretPhrase([], wordArray, anagramLetterHistogram, 1);

	return secretPhrase;
}

/**
 * Recursive function to find the secret phrase
 * 	
 * @param  {Array} wordArray              The current combination of words being tested against the anagram
 * @param  {Array} wordList               The list of all Words
 * @param  {Array} anagramLetterHistogram Character histogram of the anagram of the secret phrase
 * @param  {Int} depth                    The current depth of the recursion
 */
function recursivelyFindSecretPhrase(wordArray, wordList, anagramLetterHistogram, depth) {
	// Add each of the words in the word list to the current word
	for (var i = 0; i < wordList.length; i++) {
		// Clone the array
		var copyArray = wordArray.slice(0);

		// Add the new word to the word 
		copyArray.push(wordList[i]);

		// Convert the current array of words into a string and to a character histogram
		var word = copyArray.join('');
		var wordHistogram = getCharacterHistogram(word);


		// The recursion should only continue if the secret phrase has not been found	
		if(!found) {
